...
def update_user(
    session: Session, user_id: int, user_data: schemas.UserUpdate
    ...
    update_data["updated_at"] = datetime.datetime.now(
        tz=timezone.utc
    )  # TODO: write this s.t. users table is updated
...
def delete_user(session: Session, user_id: int):
    ...
    statement = (
        sa.delete(models.User)
        .where(models.User.id == user_id)
        .returning(models.User.name, models.User.id)
    )
